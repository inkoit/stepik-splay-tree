...
{
    EXPECT_FALSE(m_tree.Contains(1)) << m_tree;

    m_tree.Insert(1);
    EXPECT_TRUE(m_tree.Contains(1)) << m_tree;

    m_tree.Insert(2);
    EXPECT_EQ(3, m_tree.Sum(1, 2 + 1)) << m_tree;

    m_tree.Insert(2);
    EXPECT_TRUE(m_tree.Contains(2)) << m_tree;

    m_tree.Remove(2);
    EXPECT_FALSE(m_tree.Contains(2)) << m_tree;
    EXPECT_EQ(1, m_tree.Sum(1, 2 + 1)) << m_tree;

    m_tree.Remove(3);
    EXPECT_FALSE(m_tree.Contains(3)) << m_tree;
...
{
    EXPECT_FALSE(m_tree.Contains(0)) << m_tree;

    m_tree.Insert(0);
    EXPECT_TRUE(m_tree.Contains(0)) << m_tree;
...
TEST_F(SplayTreeCases, Case3)
{
    ...
    EXPECT_TRUE(m_tree.Contains(491572259)) << m_tree;
    EXPECT_FALSE(m_tree.Contains(899375874)) << m_tree;
...
TEST_F(SplayTreeCases, Case5)
{
    ...
    EXPECT_TRUE(m_tree.Contains(100500));
    m_tree.Insert(100501);
    EXPECT_TRUE(m_tree.Contains(100501));
    m_tree.Remove(100500);
    m_tree.Insert(100599);
    EXPECT_FALSE(m_tree.Contains(100500));
    m_tree.Insert(2);
    m_tree.Insert(3);
    m_tree.Insert(4);
    m_tree.Insert(7);
